...
### 5. 백준 2667번: 단지번호붙이기
--------------------------
![image](https://user-images.githubusercontent.com/67541842/89726593-fe7f0000-da56-11ea-98d5-bde82c9eabdc.png)

#### 설명:         
        
          
    이 문제는 DFS를 활용하여 풀었다. 아파트 현황을 나타내는 arr 이외에 visit라는 2차원배열을 선언하고       
    초기값으로 0을 둔다. 재귀를 이용하여 DFS함수를 실행하며 visit 배열에 번호를 부여하는데 인접한 아파트에는 같은 번호가 부여되도록 한다.
    반복문에 의해 단지번호가 하나씩 증가하며 부여되기 때문에 결국 num 변수에는 단지의 개수가 저장된다.
    size 배열을 선언하고 visit배열을 순차적으로 검색하고 부여된 단지번호-1을 인덱스번호로 하여 같은 번호를 가진       
    아파트의 수를 1씩 늘려가며 저장한다. size 배열을 반복문을 활용하여 오름차순으로 정렬하고      
    출력 예제의 형식과 같이 출력되도록 구현하였다.
